...
import { ArrowLeft, Calendar, MapPin, Ticket as TicketIcon, Film, Sparkles, AlertTriangle } from "lucide-react";
...
import { SeatSelector, ReservationType } from "@/components/events/seat-selector";
  ...
  const reserveMutation = useMutation({
    mutationFn: () => {
      ...
      if (selection.type === "seat" && !selection.seatCode) {
        throw new Error("Por favor, selecione um assento no mapa antes de prosseguir.");
      }
    ...
    onSuccess: (reservation) => {
      ...
      toast.success("Reserva realizada com sucesso!", {
        description: "Seu assento foi garantido. Conclua o checkout para emitir os bilhetes.",
      });
    },
    onError: (err) => {
      ...
      if (axios.isAxiosError(err) && err.response?.status === 409) {
        toast.error("Este assento acabou de ser reservado por outro cliente", {
          description: "Por favor, escolha outro lugar disponível no mapa de assentos.",
        });
  ...
  if (isError || !event) {
    return (
      <Card className="p-8 text-center space-y-4 max-w-lg mx-auto border-destructive/30 bg-destructive/5">
        ...
        <CardDescription>
          {error ? getApiErrorMessage(error) : "Não foi possível carregar as informações deste evento."}
        </CardDescription>
        <div className="pt-2">
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-xl bg-card/60 border border-border">
            ...
            <div>
              <span className="text-xs text-muted-foreground block">Capacidade Total</span>
              <span className="text-sm font-semibold text-foreground flex items-center gap-1 mt-0.5">
            ...
            <div>
              <span className="text-xs text-muted-foreground block">Valor Unitário</span>
